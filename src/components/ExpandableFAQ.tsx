...
export default async function ExpandableFAQ() {
    ...
    return (
        <div className="expandable-faq page-container pt-16">
            <h2 className="text-4xl font-bold text-center pb-12">Frequently asked questions</h2>
            <dl className="pb-10">
                {faq.map(({ question, answer }, index) => (
                    <div key={index} className="expandable-faq-item cursor-pointer border border-gray-200 rounded-xl pb-3 mb-3">
                        ...
                        <dd className="px-6 overflow-hidden h-0">
                            <div className="expandable-faq-content" style={{ maxWidth: "45em" }} dangerouslySetInnerHTML={{ __html: answer }} />
                        </dd>
                    </div>
                ))
                }
